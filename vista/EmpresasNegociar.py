...
class Aplicacion(Frame):
	def __init__(self, master,arreglo,seleccionado,jugador,negociante,mercado_de_valores):
            ...
            self.botonEnviar=Button(self.master, text="vender",command=lambda:negociar(self.master,jugador,negociante,mercado_de_valores,self.valores[self.idEmpresa.get()]),font=("",40))
...
def creavalores(arreglo,jugador,negociante,mercado_de_valores):
    titulaciones= negociante.titulos
    for titulo in titulaciones:
        if(negociante.posicion in range(1,6) and jugador.titulos[titulo]>0):
            if(int(titulo) in range(1,6)):
                empresa=Empresas(titulo,mercado_de_valores.empresas[titulo].nombre)
                arreglo.append(empresa)
        if(negociante.posicion in range(7,12) and jugador.titulos[titulo]>0):
            if(int(titulo) in range(7,12)):
                empresa=Empresas(titulo,mercado_de_valores.empresas[titulo].nombre)
                arreglo.append(empresa)
        if(negociante.posicion in range(13,18) and jugador.titulos[titulo]>0):
            if(int(titulo) in range(13,18)):
                empresa=Empresas(titulo,mercado_de_valores.empresas[titulo].nombre)
                arreglo.append(empresa)
        if(negociante.posicion in range(19,24) and jugador.titulos[titulo]>0):
            if(int(titulo) in range(19,24)):
                empresa=Empresas(titulo,mercado_de_valores.empresas[titulo].nombre)
                arreglo.append(empresa)

def EmpresasNegociarvista(ventana,jugador,negociante,mercado_de_valores):
    ventana.destroy()
    arreglo2 = []
    seleccionado = 1
    creavalores(arreglo2,jugador,negociante,mercado_de_valores)
    root = Tk()
    root.resizable(0,0)
    a = Aplicacion(root,arreglo2,seleccionado,jugador,negociante,mercado_de_valores)
    root.mainloop()